...
static NSString *storeItemCellID = @"DCStoreItemCell";

@interface ViewController ()<UITableViewDelegate , UITableViewDataSource,SDCycleScrollViewDelegate,UISearchBarDelegate>

@property (nonatomic , strong) UITableView *tableView;
/* 数据 */
@property (strong , nonatomic)NSMutableArray<ProductModel *> *storeItem;
/* 小标题 */
...
#pragma mark - 懒加载
- (NSMutableArray<ProductModel *> *)storeItem
{
    if (!_storeItem) {
...
- (void)viewDidLoad {
    ...
    [self setUpHeaderView:@[@"xiaomi6.png",@"mainHead"] WithAdvertisement:@"这里是我的github地址，欢迎浏览"];
}
...
- (void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index {
    ...
    if (index == 0) {
        ...
        bannerWebVc.url = @"https://www.jd.com/";
...
- (void)customButtonClick
{
    ...
    //点击当前界面返回
    ScreenVC *shopsCustomVc = [[ScreenVC alloc] init];
...
- (void)loadStoreDatas
{
    ...
    _storeItem = [ProductModel mj_objectArrayWithKeyValuesArray:storeArray];
    NSLog(@"________%@",_storeItem);
...
{
    DCStoreItemCell *cell = [tableView dequeueReusableCellWithIdentifier:storeItemCellID forIndexPath:indexPath];
    cell.storeItem = _storeItem[indexPath.row];
    NSLog(@"__________%@",_storeItem[indexPath.row]);
    return cell;
}
...
{
    ProductDetailVC *dcStoreDetailVc = [[ProductDetailVC alloc] initWithTransitionStyle:UIPageViewControllerTransitionStyleScroll navigationOrientation:UIPageViewControllerNavigationOrientationHorizontal options:nil];
...
- (void)setUpTab
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([DCStoreItemCell class]) bundle:nil] forCellReuseIdentifier:storeItemCellID];
}
